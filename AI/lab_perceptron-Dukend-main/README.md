...
```
Confusion matrix:
[[786   2   3   1   1  11  14   1   0   2]
 [  0 886   3   7   2   3   3   1  12   1]
...
Персептрон плохо справлялся с цветными картинками. Дело в том, что персептрон в принципе сам по себе плохо приспособлен для работы с изображениями в явном виде. Он никак не учитывает пространственную информацию картинки и плохо работает с цветными изображениями из-за трех каналов. А также получается довольно большое количество параметров (784*10+10 для однослойного персептрона и мнист). Поэтому лучше прогонять картинки сначала через свёрточную сеть, чтобы получить признаки для полносвязной сети. Таких знаков будет намного меньше, и в то же время они будут нести гораздо больше информации.
...
В этой работе я написал собственную библиотеку для обучения нейронных сетей. Конечно, его еще можно улучшать и улучшать — можно добавлять сверточные и рекуррентные слои, дополнительные функции активации, методы регуляризации, функции потерь для регрессии и т. д. Но даже с небольшим подмножеством «объектов» нейросетей, которые я описал , мне удалось получить хорошую библиотеку, которую легко расширять. Я успешно протестировал свою библиотеку и использовал ее для написания классификаторов для черно-белых чисел и деталей одежды.

Во второй части работы я познакомился с одним из самых популярных фреймворков для обучения нейронных сетей — PyTorch. Он более гибкий по сравнению с TensorFlow/Keras. При этом в нем уже реализованы все необходимые компоненты для работы с нейросетями, данными, картинками, текстами. Я использовал более продвинутые методы и поэтому получил более точную классификацию чисел и одежды.

Вообще нейронные сети — очень мощный механизм. Они продолжают удивлять нас каждый день. Так что понимание нейронных сетей сегодня — очень полезный и востребованный навык.
